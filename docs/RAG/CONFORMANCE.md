...
- **Firefox:** the cross-browser core suite against the production build: state/persistence, a Running real AudioWorklet, repeated start/stop, analyzer sampling/teardown, and privacy-safe sharing.
...
GitHub-hosted Linux runners do not provide a physical audio device. CI therefore starts a PulseAudio null sink before the browser matrix. This is test infrastructure, not a Greygen runtime fallback: the browser still constructs and runs its native `AudioContext`/`AudioWorklet` graph, but has a deterministic output endpoint so Firefox does not stall waiting for unavailable host audio. The null sink validates browser lifecycle, processor execution, analyzer sampling, and cleanup; it does not claim that CI has acoustically validated audible hardware output.
...
3. a real AudioWorklet reaches Running after an explicit keyboard user gesture;
4. three consecutive start/stop cycles close cleanly, with analyzer sampling/teardown exercised during the first cycle;
...
The lifecycle journey uses keyboard activation for Start/Stop so a waiting service-worker update banner cannot intercept pointer input and create a false audio-lifecycle failure. The activation remains an explicit user gesture and also exercises the keyboard-accessible transport contract from issue #18.
...
- current desktop Firefox: Start, confirm output is audible, change a band/width, Stop, restart;
...
From a clean checkout with the pinned Node/npm versions and a usable local audio output:
...
Headless Linux environments without an audio output should provision an OS-level virtual sink as the GitHub Actions workflow does; do not add a synthetic Web Audio implementation or weaken Running/lifecycle assertions just to accommodate the host.
